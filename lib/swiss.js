...
//
// Excepción: si un jugador tiene MÁS DE UN AUTOWIN/AUTOLOSE en total,
// el primero de ellos (en orden cronológico de ronda) no cuenta ni
// como ronda jugada ni como ganada — sigue siendo el "bye" normal de
// cualquier torneo suizo. A partir del segundo AUTOWIN/AUTOLOSE que
// reciba ese mismo jugador (por ejemplo, por registrarse tarde y
// toparse con más de una ronda ya en curso), esos sí cuentan como
// ronda jugada (y ganada, si fue AUTOWIN) igual que una partida real.
// Si el jugador solo tiene UN auto en todo el torneo, se sigue
// contando normal, sin excepción.
function ownRecord(rounds) {
  const sorted = [...rounds].sort((a, b) => a.roundNumber - b.roundNumber);
  const autoIndices = [];
  sorted.forEach((r, i) => {
    if (r.opponentId == null) autoIndices.push(i);
  });

  const counted = autoIndices.length > 1 ? sorted.filter((_, i) => i !== autoIndices[0]) : sorted;

  const played = counted.length;
  if (!played) return 0;
  const wins = counted.filter((r) => r.outcome === "win").length;
  const draws = counted.filter((r) => r.outcome === "draw").length;
