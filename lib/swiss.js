...
// cronológico de ronda) siempre se omite del cálculo de winrate — ni
// cuenta como ronda jugada ni como ganada, aunque haya sido AUTOWIN.
// (Los puntos de standings sí lo siguen contando como victoria normal,
// eso no cambia — esto solo afecta el winrate/record.)
// A partir del SEGUNDO AUTOWIN/AUTOLOSE que reciba ese mismo jugador,
// esos sí cuentan como ronda jugada (y ganada, si fue AUTOWIN) igual
// que una partida real.
function ownRecord(rounds) {
  const sorted = [...rounds].sort((a, b) => a.roundNumber - b.roundNumber);
  const firstAutoIndex = sorted.findIndex((r) => r.opponentId == null);
  const counted = firstAutoIndex === -1 ? sorted : sorted.filter((_, i) => i !== firstAutoIndex);
