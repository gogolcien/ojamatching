...
// Determina, para la lista de rondas de un jugador (ordenadas), el
// índice del primer AUTOWIN/AUTOLOSE (o -1 si no tuvo ninguno). Ese
// primer auto es el único que se excluye de "rondas jugadas" en todos
// los cálculos de winrate/OP%/OOP% de este archivo.
function firstAutoIndexOf(sortedRounds) {
  return sortedRounds.findIndex((r) => r.opponentId == null);
}

// Cuántas rondas cuentan como "jugadas" para un jugador: todas menos
// el primer AUTOWIN/AUTOLOSE (si tuvo alguno). Los AUTOWIN/AUTOLOSE
// del segundo en adelante sí cuentan como jugadas, igual que una
// partida real.
function ownPlayedCount(rounds) {
  if (!rounds.length) return 0;
  const firstAutoIndex = firstAutoIndexOf(rounds);
  return firstAutoIndex === -1 ? rounds.length : rounds.length - 1;
}
...
//
// El PRIMER AUTOWIN/AUTOLOSE que recibe un jugador (en orden
// cronológico de ronda) no cuenta como ronda jugada (no entra al
// denominador) — PERO si fue un AUTOWIN, su victoria sí se toma en
// cuenta en el numerador (rondas ganadas). Si fue un AUTOLOSE, no
// suma nada (una derrota tampoco sumaría en un caso normal).
// A partir del SEGUNDO AUTOWIN/AUTOLOSE que reciba ese mismo jugador,
// esos cuentan por completo (jugada y, si aplica, ganada) igual que
// una partida real. Los puntos de standings no se ven afectados por
// nada de esto: siguen contando cada AUTOWIN como victoria normal.
function ownRecord(rounds) {
  const sorted = [...rounds].sort((a, b) => a.roundNumber - b.roundNumber);
  const played = ownPlayedCount(sorted);
  if (!played) return 0;

  // El numerador suma TODAS las victorias (incluida la del primer
  // auto si fue AUTOWIN); solo el denominador excluye ese primer auto.
  let wins = 0;
  let draws = 0;
  sorted.forEach((r) => {
    if (r.outcome === "win") wins += 1;
    else if (r.outcome === "draw") draws += 1;
  });
...
function computeStats(tournament) {
  ...
  // de AUTOWIN/AUTOLOSE, que no tienen rival), pero dividido entre las
  // "rondas jugadas" propias del jugador (mismo denominador que su
  // winrate) — no entre la cantidad de rivales reales que tuvo. Así,
  // si el jugador tuvo 2 rivales reales y 2 rondas jugadas contando
  // solo uno de sus autos, el divisor es 3 (2 reales + 1 auto que sí
  // cuenta como jugada), no 2.
  const opPercent = {};
  ids.forEach((id) => {
    const rounds = roundsByPlayer[id];
    const played = ownPlayedCount(rounds);
    if (!played) {
      opPercent[id] = 0;
      return;
    }
    const opponentIds = rounds.filter((r) => r.opponentId != null).map((r) => r.opponentId);
    const sum = opponentIds.reduce((s, oid) => s + (record[oid] ?? 0), 0);
    opPercent[id] = sum / played;
  });

  // OOP%: promedio del OP% de cada rival real, con el mismo criterio
  // de divisor que OP% (rondas jugadas propias, no cantidad de rivales).
  const oopPercent = {};
  ids.forEach((id) => {
    const rounds = roundsByPlayer[id];
    const played = ownPlayedCount(rounds);
