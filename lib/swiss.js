...
// Cuántas rondas cuentan como "jugadas" para un jugador, a efectos de
// winrate/OP%/OOP%:
//   - Si tiene 0 o 1 AUTOWIN/AUTOLOSE en total: TODAS sus rondas
//     cuentan como jugadas, sin excepción (el auto cuenta normal).
//   - Si tiene 2 o más AUTOWIN/AUTOLOSE en total: se excluye
//     exactamente UNA de esas rondas auto del denominador (no importa
//     cuál específicamente, ya que el numerador de victorias — ver
//     ownRecord — nunca se ve afectado por esta exclusión). Las
//     demás autos cuentan normal, igual que una partida real.
function ownPlayedCount(rounds) {
  if (!rounds.length) return 0;
  const totalAutos = rounds.filter((r) => r.opponentId == null).length;
  return totalAutos >= 2 ? rounds.length - 1 : rounds.length;
}
...
//
// El numerador (rondas ganadas) SIEMPRE suma todas las victorias
// reales y todos los AUTOWIN, sin excepción. Solo el denominador
// (rondas jugadas, ver ownPlayedCount) se reduce en 1 cuando el
// jugador tiene 2+ AUTOWIN/AUTOLOSE en total. Los puntos de standings
// no se ven afectados por nada de esto: siguen contando cada AUTOWIN
// como victoria normal (3 pts).
