...
// Record propio de un jugador: rondas ganadas / número de rondas
// (donde un empate cuenta como 1/3 de triunfo, misma proporción que
// 1 de los 3 puntos que da una victoria). Cada AUTOWIN/AUTOLOSE
// cuenta exactamente igual que una partida real, sin excepciones.
// Trunca una fracción (0-1) a 2 decimales de porcentaje (4 decimales de
// fracción), sin redondear. El pequeño epsilon evita que errores de punto
// flotante (p.ej. 0.5759999999999998 en vez de 0.576) le resten una centésima
// al truncar. Este es el valor que se usa TANTO para ordenar standings COMO
// para mostrar en pantalla, para que nunca se vean desalineados.
function truncatePercent(x) {
  return Math.trunc(x * 10000 + 1e-9) / 10000;
}
...
function ownRecord(rounds) {
  ...
  return truncatePercent((wins + draws / 3) / played);
}

// Calcula puntos, record propio, OW% (Opponents Winrate), OOW%
// (Opponents' Opponents Winrate) y SL para todos los jugadores de un
// torneo, a partir de todas las rondas ya resueltas.
function computeStats(tournament) {
  ...
  // OW% (Opponents Winrate): promedio del winrate de cada RIVAL REAL,
  // dividido entre la cantidad de rivales reales que tuvo (se omiten
  // por completo las rondas de AUTOWIN/AUTOLOSE, tanto de la suma
  // como del divisor, porque no hay rival real en esas rondas).
  const owPercent = {};
  ids.forEach((id) => {
    ...
    owPercent[id] = truncatePercent(sum / opponentIds.length);
  });

  // OOW% (Opponents' Opponents Winrate): promedio del OW% de cada
  // rival real, mismo criterio de omitir AUTOWIN/AUTOLOSE.
  const oowPercent = {};
  ids.forEach((id) => {
    const opponentIds = roundsByPlayer[id]
      .filter((r) => r.opponentId != null)
    ...
    const sum = opponentIds.reduce((s, oid) => s + (owPercent[oid] ?? 0), 0);
    oowPercent[id] = truncatePercent(sum / opponentIds.length);
  });
  ...
  return { points, record, owPercent, oowPercent, sl, roundsPlayed, opponentsOf, roundsByPlayer };
}
...
function computeStandings(tournament) {
  ...
  const rows = tournament.players.map((p) => ({
    ...
    owPercent: stats.owPercent[p.id] || 0,
    oowPercent: stats.oowPercent[p.id] || 0,
    sl: stats.sl[p.id] || 0,
  ...
  rows.sort(
    (a, b) =>
      b.points - a.points ||
      b.owPercent - a.owPercent ||
      b.oowPercent - a.oowPercent ||
      b.sl - a.sl ||
...
export { computeStandings, computeStats, generatePairings, isRoundFullyResolved, truncatePercent };
