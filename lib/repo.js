...
// Crea un nuevo torneo de formato 'elimination' ("Top Cut") a partir
// de los N mejores jugadores (ya ordenados por standings) de un
// torneo suizo. Copia esos jugadores como nuevos registros en el
// torneo nuevo (nombre y deck; no hereda puntos ni historial, porque
// el Top Cut siempre arranca de cero) y genera su Ronda 1 ya sembrada
// (ver seedTopCutFirstRound en lib/bracket.js), en vez de al azar.
// Devuelve el id del torneo nuevo.
export async function createTopCutTournament({ sourceTournamentId, name, date, orderedTopPlayers }) {
  const db = await getDb();
  const { seedTopCutFirstRound } = await import("./bracket");

  const newId = uuid();
  await db.runAsync(
    `INSERT INTO tournaments (id, name, date, format, status, created_at, source_tournament_id)
     VALUES (?, ?, ?, 'elimination', 'ongoing', ?, ?)`,
    [newId, name.trim(), date, new Date().toISOString(), sourceTournamentId]
  );

  const newPlayers = [];
  for (let i = 0; i < orderedTopPlayers.length; i++) {
    const src = orderedTopPlayers[i];
    const pid = uuid();
    await db.runAsync(
      `INSERT INTO players (id, tournament_id, name, deck, enabled, eliminated, sort_order) VALUES (?, ?, ?, ?, 1, 0, ?)`,
      [pid, newId, src.name, src.deck ?? null, i]
    );
    newPlayers.push({ id: pid });
  }

  const pairs = seedTopCutFirstRound(newPlayers, newPlayers.length);
  await createRound(newId, 1, pairs);
    ...
    rounds.map(async (r) => ({
      ...
      matches: (
        await db.getAllAsync(
          `SELECT id, table_num AS tableNum, slot_index AS slotIndex, player_a_id AS playerAId, player_b_id AS playerBId, result, is_third_place AS isThirdPlace
...
// pairs: [{ slotIndex, playerAId, playerBId, isThirdPlace? }] (playerBId null = AUTOWIN)
export async function createRound(tournamentId, roundNumber, pairs) {
  ...
  for (const p of pairs) {
    ...
    await db.runAsync(
      `INSERT INTO matches (id, round_id, table_num, slot_index, player_a_id, player_b_id, result, is_third_place)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [uuid(), roundId, table++, p.slotIndex ?? null, p.playerAId, p.playerBId, result, p.isThirdPlace ? 1 : 0]
    );
  }
  return roundId;
}

// Agrega una mesa AUTOWIN a una ronda que ya fue generada, para un
// jugador que se registró después de que se hizo el pareo. Se agrega
// como mesa extra al final (no reemplaza nada de lo ya pareado), por
// lo que una misma ronda puede terminar con más de un AUTOWIN/AUTOLOSE
// si se registran varios jugadores tarde.
export async function addLateAutowin(roundId, playerId) {
  const db = await getDb();
  const row = await db.getFirstAsync(
    `SELECT COALESCE(MAX(table_num), 0) + 1 AS next FROM matches WHERE round_id = ?`,
    [roundId]
  );
  await db.runAsync(
    `INSERT INTO matches (id, round_id, table_num, slot_index, player_a_id, player_b_id, result)
     VALUES (?, ?, ?, ?, ?, ?, 'bye_win')`,
    [uuid(), roundId, row.next, null, playerId, null]
  );
}
